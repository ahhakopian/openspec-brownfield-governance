...
class DoctorTests(TempProjectTest):
    ...
    def test_doctor_reports_modified_skill_as_unhealthy(self):
        invoke(self.root, self.env, "install")
        skill = self.root / ".agents/skills/brownfield-complexity-gate/SKILL.md"
    ...
    def test_doctor_reports_missing_complexity_gate_as_unhealthy(self):
        invoke(self.root, self.env, "install")
        (self.root / ".agents/skills/brownfield-complexity-gate/SKILL.md").unlink()

        result = invoke(self.root, self.env, "doctor", check=False)

        self.assertEqual(result.returncode, 1)
        self.assertIn("missing installed skill", result.stdout)

    def test_doctor_reports_duplicate_owned_config_item_as_unhealthy(self):
        invoke(self.root, self.env, "install")
        config = self.root / "openspec/config.yaml"
        item_line = "      - " + json.dumps(CONTRACT["operations"]["apply"][1])
        ...
        self.assertEqual(result.returncode, 1)
        self.assertIn("Installation health: UNHEALTHY", result.stdout)
        self.assertIn("operations.apply.guidance duplicates 1 required item", result.stdout)
